...
unsafe extern "C" fn annexb_probe(mut data: *const uint8_t) -> libc::c_int {
    ...
    if ret < 0i32 || frame_unit_size.wrapping_add(ret as libc::c_ulong) > temporal_unit_size {
        return 0i32;
    }
    cnt += ret;
    temporal_unit_size = (temporal_unit_size).wrapping_sub(ret as libc::c_ulong);
    let mut obu_unit_size: size_t = 0;
    ret = leb(data.offset(cnt as isize), 2048i32 - cnt, &mut obu_unit_size);
    if ret < 0i32 || obu_unit_size.wrapping_add(ret as libc::c_ulong) >= frame_unit_size {
        return 0i32;
    }
    cnt += ret;
    temporal_unit_size =
        (temporal_unit_size).wrapping_sub(obu_unit_size.wrapping_add(ret as libc::c_ulong));
    frame_unit_size =
        (frame_unit_size).wrapping_sub(obu_unit_size.wrapping_add(ret as libc::c_ulong));
    let mut obu_size: size_t = 0;
    let mut type_0: Dav1dObuType = 0u32;
    ret = parse_obu_header(
        data.offset(cnt as isize),
        imin(2048i32 - cnt, obu_unit_size as libc::c_int),
        &mut obu_size,
        &mut type_0,
        1i32,
    );
    ...
    while cnt < 2048i32 {
        ...
        cnt += ret;
        temporal_unit_size = (temporal_unit_size).wrapping_sub(ret as libc::c_ulong);
        frame_unit_size = (frame_unit_size).wrapping_sub(ret as libc::c_ulong);
        ret = parse_obu_header(
            data.offset(cnt as isize),
            imin(2048i32 - cnt, obu_unit_size as libc::c_int),
            &mut obu_size,
        ...
        frame_unit_size = (frame_unit_size).wrapping_sub(obu_unit_size);
        if frame_unit_size <= 0u64 {
            return 0i32;
        }
    }
    return seq;
...
unsafe extern "C" fn annexb_read(c: *mut AnnexbInputContext, data: *mut Dav1dData) -> libc::c_int {
    ...
    if (*c).frame_unit_size == 0 {
        ...
        if res < 0i32
            || ((*c).frame_unit_size).wrapping_add(res as libc::c_ulong) > (*c).temporal_unit_size
        {
            return -(1i32);
        }
        (*c).temporal_unit_size = ((*c).temporal_unit_size).wrapping_sub(res as libc::c_ulong);
    }
    res = leb128((*c).f, &mut len);
    if res < 0i32 || len.wrapping_add(res as libc::c_ulong) > (*c).frame_unit_size {
        return -(1i32);
    }
    ...
    (*c).temporal_unit_size =
        ((*c).temporal_unit_size).wrapping_sub(len.wrapping_add(res as libc::c_ulong));
    (*c).frame_unit_size =
        ((*c).frame_unit_size).wrapping_sub(len.wrapping_add(res as libc::c_ulong));
    if fread(ptr as *mut libc::c_void, len, 1u64, (*c).f) != 1u64 {
        fprintf(
            stderr,
...
pub static mut annexb_demuxer: Demuxer = unsafe {
    {
        let mut init = Demuxer {
            priv_data_size: ::core::mem::size_of::<AnnexbInputContext>() as libc::c_int,

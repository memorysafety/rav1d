...
unsafe extern "C" fn padding(
    ...
    if edges as libc::c_uint & LR_HAVE_BOTTOM as libc::c_int as libc::c_uint != 0 {
        ...
        let src: *const pixel = p
            .offset(((stripe_h - 1) as isize * stride) as isize);
        memcpy(
            dst_tl.offset((stripe_h * 390) as isize) as *mut libc::c_void,
...
unsafe extern "C" fn wiener_c(
    ...
    while j < h + 6 {
        ...
        while i < w {
            ...
            sum
                += *tmp_ptr.offset((i + 3) as isize) as libc::c_int
                    * 128;
            let mut k = 0;
            while k < 7 {
                sum
...
unsafe extern "C" fn selfguided_filter(
    ...
    if n == 25 {
        ...
        while j_0 < h - 1 {
            ...
            let mut i_1 = 0;
            while i_1 < w {
                let a_1 = *B.offset(i_1 as isize) as libc::c_int
                    * 6
...
unsafe extern "C" fn sgr_5x5_c(
    ...
    while j < h {
        ...
        while i < w {
            let v = w0
                * dst[(j * 384 + i) as usize] as libc::c_int;
            *p
                .offset(
                    i as isize,
                ) = iclip_u8(
...
unsafe extern "C" fn sgr_mix_c(
    ...
    while j < h {
        ...
        while i < w {
            let v = w0
                * dst0[(j * 384 + i) as usize] as libc::c_int

...
/// create a mutable pointer to the underlying slice without creating a mutable
/// reference to the slice without ever creating a mutable reference to it.
///
/// # Safety
///
...
impl<T: AsMutPtr> DisjointMut<T> {
    pub fn len(&self) -> usize {
        unsafe { (*self.inner.get().cast_const()).len() }
    }
        ...
        impl<T> DisjointMutIndex<[T]> for $range_type {
            ...
            unsafe fn get_mut(self, slice: *mut [T]) -> *mut Self::Output {
                ...
                if start <= end && start < len && end <= len {
                    unsafe { slice::from_raw_parts_mut((slice as *mut T).add(start), end - start) }
                } else {
                    panic!("{:?} was not a valid index", &self);
                }
            }
...
mod release {
    ...
    impl<T: AsMutPtr> DisjointMut<T> {
        ...
        ///
        /// This mutable borrow is unchecked and callers must ensure that no
        /// other borrows from this collection overlap with the mutably borrowed
        /// region for the lifetime of that mutable borrow.
        ///
        /// # Safety
        ///
        /// Caller must ensure that no elements of the resulting borrowed slice
        /// or element is concurrently borrowed (immutably or mutably) at all
        /// during the lifetime of the returned mutable borrow.
        pub unsafe fn index_mut<I>(
            &self,
            index: I,
        ) -> &mut <[<T as AsMutPtr>::Target] as Index<I>>::Output
        where
            ...
            >,
        {
            // SAFETY: The preconditions of our `index_mut` safety imply that
            // the indexed region we are mutably borrowing is not concurrently
            // borrowed and will not be borrowed during the lifetime of the
            // returned reference.
            unsafe { &mut *index.get_mut(self.as_mut_slice()) }
        }

        pub fn index<I>(&self, index: I) -> &<[<T as AsMutPtr>::Target] as Index<I>>::Output
        where
        ...
        {
            // SAFETY: The preconditions of our `index_mut` safety imply that
            // the indexed region is not mutably borrowed and will not be
            // mutably borrowed during the lifetime of the returned reference.
            unsafe { &*index.get_mut(self.as_mut_slice()).cast_const() }
        }
    }
}
...
mod debug {
    ...
    impl<T: AsMutPtr> DisjointMut<T> {
        ...
        where
            ...
            >,
        {
            self.add_mut_range(&index);
            let range = index.to_bounds();
            // SAFETY: `add_mut_range` above ensures that we have no other
            // borrows that overlap with this borrowed range.
        ...
        where
            ...
            >,
        {
            self.add_immut_range(&index);
            let range = index.to_bounds();
            // SAFETY: `add_immut_range` above ensures that we have no mutable
            // borrows that overlap with this borrowed range.
...
unsafe impl<V> AsMutPtr for Vec<V> {
    ...
    unsafe fn as_mut_slice(ptr: *mut Self) -> *mut [Self::Target] {
        let len = unsafe { (*ptr.cast_const()).len() };
...
impl<V, const A: usize, const B: usize> DisjointMut<[[V; A]; B]> {
    // TODO(SJC): Add debug ranges for this method or make this return a raw pointer?
    #[allow(unused)]
    pub unsafe fn nd_index_mut(&self, index: [usize; 2]) -> &mut V {
        assert!(index[0] < B);
        assert!(index[1] < A);
        let array = self.inner.get() as *mut [V; A];
        let array = array.wrapping_add(index[0]) as *mut V;
        // SAFETY: The preconditions of our `index_mut`  and `nd_index_mut`
        // safety imply that the indexed region we are mutably borrowing is not
        // concurrently borrowed and will not be borrowed during the lifetime of
        // the returned reference. We ensure that the indices are in-bounds with
        // the two asserts above.
        unsafe { &mut *array.wrapping_add(index[1]) }
    }
}
...
#[test]
fn test_nd_index_mut() {
    let v: DisjointMut<[[u8; 4]; 10]> = Default::default();

    let borrow = v.index(0..);
    unsafe {
        *v.nd_index_mut([3, 2]) = 42;
    }
    assert_eq!(borrow[4][0], 0);

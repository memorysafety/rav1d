use ::libc;
pub type __uint8_t = libc::c_uchar;
pub type __int64_t = libc::c_long;
pub type __uint64_t = libc::c_ulong;
pub type int64_t = __int64_t;
pub type uint8_t = __uint8_t;
pub type uint64_t = __uint64_t;
pub type size_t = libc::c_ulong;

#[repr(C)]
#[derive(Copy, Clone)]
pub struct GetBits {
    pub state: uint64_t,
    pub bits_left: libc::c_int,
    pub error: libc::c_int,
    pub ptr: *const uint8_t,
    pub ptr_start: *const uint8_t,
    pub ptr_end: *const uint8_t,
}
#[inline]
unsafe extern "C" fn inv_recenter(r: libc::c_uint, v: libc::c_uint) -> libc::c_uint {
    if v > r << 1i32 {
        return v;
    } else if v & 1u32 == 0u32 {
        return (v >> 1i32).wrapping_add(r);
    } else {
        return r.wrapping_sub(v.wrapping_add(1u32) >> 1i32);
    };
}
#[inline]
unsafe extern "C" fn ulog2(v: libc::c_uint) -> libc::c_int {
    return 31i32 - clz(v);
}
#[inline]
unsafe extern "C" fn clz(mask: libc::c_uint) -> libc::c_int {
    return mask.leading_zeros() as i32;
}
#[no_mangle]
pub unsafe extern "C" fn dav1d_init_get_bits(c: *mut GetBits, data: *const uint8_t, sz: size_t) {
    if sz == 0 {
        unreachable!();
    }
    (*c).ptr_start = data;
    (*c).ptr = (*c).ptr_start;
    (*c).ptr_end = &*((*c).ptr_start).offset(sz as isize) as *const uint8_t;
    (*c).state = 0u64;
    (*c).bits_left = 0i32;
    (*c).error = 0i32;
}
#[no_mangle]
pub unsafe extern "C" fn dav1d_get_bit(c: *mut GetBits) -> libc::c_uint {
    if (*c).bits_left == 0 {
        if (*c).ptr >= (*c).ptr_end {
            (*c).error = 1i32;
        } else {
            let fresh0 = (*c).ptr;
            (*c).ptr = ((*c).ptr).offset(1);
            let state: libc::c_uint = *fresh0 as libc::c_uint;
            (*c).bits_left = 7i32;
            (*c).state = (state as uint64_t) << 57i32;
            return state >> 7i32;
        }
    }
    let state_0: uint64_t = (*c).state;
    (*c).bits_left -= 1;
    (*c).state = state_0 << 1i32;
    return (state_0 >> 63i32) as libc::c_uint;
}
#[inline]
unsafe extern "C" fn refill(c: *mut GetBits, n: libc::c_int) {
    if !((*c).bits_left >= 0i32 && (*c).bits_left < 32i32) {
        unreachable!();
    }
    let mut state: libc::c_uint = 0u32;
    loop {
        if (*c).ptr >= (*c).ptr_end {
            (*c).error = 1i32;
            if state != 0 {
                break;
            }
            return;
        } else {
            let fresh1 = (*c).ptr;
            (*c).ptr = ((*c).ptr).offset(1);
            state = state << 8i32 | *fresh1 as libc::c_uint;
            (*c).bits_left += 8i32;
            if !(n > (*c).bits_left) {
                break;
            }
        }
    }
    (*c).state |= (state as uint64_t) << 64i32 - (*c).bits_left;
}
#[no_mangle]
pub unsafe extern "C" fn dav1d_get_bits(c: *mut GetBits, n: libc::c_int) -> libc::c_uint {
    if !(n > 0i32 && n <= 32i32) {
        unreachable!();
    }
    if n as libc::c_uint > (*c).bits_left as libc::c_uint {
        refill(c, n);
    }
    let state: uint64_t = (*c).state;
    (*c).bits_left -= n;
    (*c).state = state << n;
    return (state >> 64i32 - n) as libc::c_uint;
}
#[no_mangle]
pub unsafe extern "C" fn dav1d_get_sbits(c: *mut GetBits, n: libc::c_int) -> libc::c_int {
    if !(n > 0i32 && n <= 32i32) {
        unreachable!();
    }
    if n as libc::c_uint > (*c).bits_left as libc::c_uint {
        refill(c, n);
    }
    let state: uint64_t = (*c).state;
    (*c).bits_left -= n;
    (*c).state = state << n;
    return (state as int64_t >> 64i32 - n) as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn dav1d_get_uleb128(c: *mut GetBits) -> libc::c_uint {
    let mut val: uint64_t = 0u64;
    let mut i: libc::c_uint = 0u32;
    let mut more: libc::c_uint = 0;
    loop {
        let v: libc::c_int = dav1d_get_bits(c, 8i32) as libc::c_int;
        more = (v & 0x80i32) as libc::c_uint;
        val |= ((v & 0x7fi32) as uint64_t) << i;
        i = i.wrapping_add(7u32);
        if !(more != 0 && i < 56u32) {
            break;
        }
    }
    if val > (2147483647u32).wrapping_mul(2u32).wrapping_add(1u32) as libc::c_ulong || more != 0 {
        (*c).error = 1i32;
        return 0u32;
    }
    return val as libc::c_uint;
}
#[no_mangle]
pub unsafe extern "C" fn dav1d_get_uniform(c: *mut GetBits, max: libc::c_uint) -> libc::c_uint {
    if !(max > 1u32) {
        unreachable!();
    }
    let l: libc::c_int = ulog2(max) + 1i32;
    if !(l > 1i32) {
        unreachable!();
    }
    let m: libc::c_uint = ((1u32) << l).wrapping_sub(max);
    let v: libc::c_uint = dav1d_get_bits(c, l - 1i32);
    return if v < m {
        v
    } else {
        (v << 1i32).wrapping_sub(m).wrapping_add(dav1d_get_bit(c))
    };
}
#[no_mangle]
pub unsafe extern "C" fn dav1d_get_vlc(c: *mut GetBits) -> libc::c_uint {
    if dav1d_get_bit(c) != 0 {
        return 0u32;
    }
    let mut n_bits: libc::c_int = 0i32;
    loop {
        n_bits += 1;
        if n_bits == 32i32 {
            return 0xffffffffu32;
        }
        if !(dav1d_get_bit(c) == 0) {
            break;
        }
    }
    return ((1u32) << n_bits)
        .wrapping_sub(1u32)
        .wrapping_add(dav1d_get_bits(c, n_bits));
}
unsafe extern "C" fn get_bits_subexp_u(
    c: *mut GetBits,
    ref_0: libc::c_uint,
    n: libc::c_uint,
) -> libc::c_uint {
    let mut v: libc::c_uint = 0u32;
    let mut i: libc::c_int = 0i32;
    loop {
        let b: libc::c_int = if i != 0 { 3i32 + i - 1i32 } else { 3i32 };
        if n < v.wrapping_add((3i32 * ((1i32) << b)) as libc::c_uint) {
            v = v.wrapping_add(dav1d_get_uniform(c, n.wrapping_sub(v).wrapping_add(1u32)));
            break;
        } else if dav1d_get_bit(c) == 0 {
            v = v.wrapping_add(dav1d_get_bits(c, b));
            break;
        } else {
            v = v.wrapping_add(((1i32) << b) as libc::c_uint);
            i += 1;
        }
    }
    return if ref_0.wrapping_mul(2u32) <= n {
        inv_recenter(ref_0, v)
    } else {
        n.wrapping_sub(inv_recenter(n.wrapping_sub(ref_0), v))
    };
}
#[no_mangle]
pub unsafe extern "C" fn dav1d_get_bits_subexp(
    c: *mut GetBits,
    ref_0: libc::c_int,
    n: libc::c_uint,
) -> libc::c_int {
    return get_bits_subexp_u(
        c,
        (ref_0 + ((1i32) << n)) as libc::c_uint,
        ((2i32) << n) as libc::c_uint,
    ) as libc::c_int
        - ((1i32) << n);
}
#[no_mangle]
pub unsafe extern "C" fn dav1d_bytealign_get_bits(mut c: *mut GetBits) {
    if !((*c).bits_left <= 7i32) {
        unreachable!();
    }
    (*c).bits_left = 0i32;
    (*c).state = 0u64;
}

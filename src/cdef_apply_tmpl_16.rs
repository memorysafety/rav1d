...
unsafe extern "C" fn backup2lines(
    ...
    if y_stride < 0 {
        memcpy(
            ...
            (-(2 as libc::c_int) as isize * y_stride << 1)
                as size_t,
        );
    } else {
        memcpy(
        ...
        if uv_stride < 0 {
            ...
            memcpy(
                ...
                (*src.offset(1))
                    ...
                    as *const libc::c_void,
                (-(2 as libc::c_int) as isize * uv_stride << 1)
                    as size_t,
            );
            memcpy(
...
pub unsafe extern "C" fn dav1d_cdef_brow_16bpc(
    ...
    let bitdepth_min_8: libc::c_int = if 16 as libc::c_int == 8 {
        0 as libc::c_int
    } else {
        (*f).cur.p.bpc - 8
    };
    ...
    let sbsz = 16;
    let sb64w: libc::c_int = (*f).sb128w << 1;
    ...
    while by < by_end {
                ...
                != 0
        {
            let cdef_top_bak: [*mut pixel; 3] = [
                ((*f)
                    ...
                    as usize][0])
                    .offset(
                        ((have_tt * sby * 4) as isize * y_stride)
                            as isize,
                    ),
                ((*f)
                    .lf
                    ...
                    as usize][1])
                    .offset(
                        ((have_tt * sby * 8) as isize * uv_stride)
                            as isize,
                    ),
                ((*f)
                    .lf
        ...
        while sbx < sb64w {
            ...
            let sb128x: libc::c_int = sbx >> 1;
            let sb64_idx: libc::c_int = ((by & sbsz) >> 3)
                + (sbx & 1);
                                ...
                                if have_tt == 0 {
                                    ...
                                    if resize != 0 {
                                        ...
                                        let line_0: libc::c_int = sby
                                            * ((4 as libc::c_int) << sb128) - 4;
                                        offset = line_0 as isize * uv_stride + (bx * 4 >> ss_hor) as isize;
                                        top = &mut *(*((*f).lf.lr_lpf_line)
                                            .as_mut_ptr()
                                            .offset(pl as isize))
                                            .offset(offset as isize) as *mut pixel;
                                    ...
                                    && by + 2 >= by_end
                                {
                                    let top_offset: ptrdiff_t = (sby * 8)
                                        as isize * uv_stride
                                        + (bx * 4 >> ss_hor) as isize;
                                    top = &mut *(*(*((*f).lf.cdef_line)
                                        .as_mut_ptr()
            ...
            iptrs[0 as libc::c_int
                as usize] = (iptrs[0])
                .offset((sbsz * 4) as isize);
            iptrs[1 as libc::c_int
                as usize] = (iptrs[1])
                .offset((sbsz * 4 >> ss_hor) as isize);

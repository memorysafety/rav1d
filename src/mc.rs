...
fn filter_8tap_mid(mid: &[i16], x: usize, f: &[i8; 8]) -> FilterResult {
    let pixel = (0..f.len())
        .map(|i| f[i] as i32 * mid[x + i * MID_STRIDE] as i32)
        .sum();
    FilterResult { pixel }
}
    ...
    if let Some(fh) = fh {
        if let Some(fv) = fv {
            ...
            for _ in 0..tmp_h {
                ...
                mid_ptr = &mut mid_ptr[MID_STRIDE..];
                src = src.offset(src_stride);
            }
            ...
            for _ in 0..h {
                ...
                for (x, dst) in dst.iter_mut().enumerate() {
                    *dst = filter_8tap_mid(mid_ptr, x, fv)
...
unsafe fn put_8tap_scaled_rust<BD: BitDepth>(
    ...
    for _ in 0..h {
        ...
        for (x, dst) in dst.iter_mut().enumerate() {
            *dst = match fv {
                Some(fv) => filter_8tap_mid(mid_ptr, x, fv)
                    .rnd(6 + intermediate_bits)
                    .clip(bd),
                None => bd.iclip_pixel(
                    (i32::from(mid_ptr[x + 3 * MID_STRIDE]) + intermediate_rnd)
                        >> intermediate_bits,
                ),
            };
        }
        ...
        my += dy;
        mid_ptr = &mut mid_ptr[(my >> 10) * MID_STRIDE..];
        my &= 0x3ff;
        dst_ptr = dst_ptr.offset(dst_stride);
    }
}
...
unsafe fn prep_8tap_rust<BD: BitDepth>(
    ...
    if let Some(fh) = fh {
        if let Some(fv) = fv {
            ...
            for _ in 0..h {
                for x in 0..w {
                    tmp[x] = filter_8tap_mid(mid_ptr, x, fv).rnd(6).sub_prep_bias::<BD>();
                }
...
unsafe fn prep_8tap_scaled_rust<BD: BitDepth>(
    ...
    for _ in 0..h {
        ...
        for x in 0..w {
            tmp[x] = match fv {
                Some(fv) => filter_8tap_mid(mid_ptr, x, fv).rnd(6),
                None => FilterResult {
                    pixel: mid_ptr[x + 3 * MID_STRIDE].into(),
                },
            }
            .sub_prep_bias::<BD>()
        }
...
fn filter_bilin_mid(mid: &[i16], x: usize, mxy: usize) -> FilterResult {
    let mid = |i| mid[i] as i32;
    let pixel = 16 * mid(x) + mxy as i32 * (mid(x + MID_STRIDE) - mid(x));
    FilterResult { pixel }
}

unsafe fn filter_bilin<BD: BitDepth>(
    src: *const BD::Pixel,
    x: usize,
    mxy: usize,
    stride: isize,
...
unsafe fn put_bilin_rust<BD: BitDepth>(
    ...
    if mx != 0 {
        if my != 0 {
            ...
            for _ in 0..tmp_h {
                for x in 0..w {
                    mid_ptr[x] = filter_bilin::<BD>(src, x, mx, 1)
            ...
            for _ in 0..h {
                let dst = slice::from_raw_parts_mut(dst_ptr, w);
                for (x, dst) in dst.iter_mut().enumerate() {
                    *dst = filter_bilin_mid(mid_ptr, x, my)
                        .rnd(4 + intermediate_bits)
                        .clip(bd)
                }
            ...
            for _ in 0..h {
                let dst = slice::from_raw_parts_mut(dst_ptr, w);
                for (x, dst) in dst.iter_mut().enumerate() {
                    *dst = filter_bilin::<BD>(src, x, mx, 1)
                        .rnd(4 - intermediate_bits)
        ...
        for _ in 0..h {
            let dst = slice::from_raw_parts_mut(dst_ptr, w);
            for (x, dst) in dst.iter_mut().enumerate() {
                *dst = filter_bilin::<BD>(src, x, my, src_stride).rnd(4).clip(bd)
            }
...
unsafe fn put_bilin_scaled_rust<BD: BitDepth>(
    ...
    for _ in 0..tmp_h {
        ...
        for x in 0..w {
            mid_ptr[x] = filter_bilin::<BD>(src, ioff, imx >> 6, 1)
                .rnd(4 - intermediate_bits)
                .get();
            imx += dx;
            ioff += imx >> 10;
    ...
    for _ in 0..h {
        let dst = slice::from_raw_parts_mut(dst_ptr, w);
        for (x, dst) in dst.iter_mut().enumerate() {
            *dst = filter_bilin_mid(mid_ptr, x, my >> 6)
                .rnd(4 + intermediate_bits)
                .clip(bd)
        }
...
unsafe fn prep_bilin_rust<BD: BitDepth>(
    ...
    if mx != 0 {
        if my != 0 {
            ...
            for _ in 0..h {
                for x in 0..w {
                    tmp[x] = filter_bilin_mid(mid_ptr, x, my)
                        .rnd(4)
                        .sub_prep_bias::<BD>()
                }
        ...
        for _ in 0..h {
            for x in 0..w {
                tmp[x] = filter_bilin::<BD>(src, x, my, src_stride)
                    .rnd(4 - intermediate_bits)
                    .sub_prep_bias::<BD>()
            }
...
unsafe fn prep_bilin_scaled_rust<BD: BitDepth>(
    ...
    for _ in 0..h {
        for x in 0..w {
            tmp[x] = filter_bilin_mid(mid_ptr, x, my >> 6)
                .rnd(4)
                .sub_prep_bias::<BD>()
        }
...
unsafe extern "C" fn put_c_erased<BD: BitDepth, const FILTER: usize>(
    ...
    match filter {
        Filter2d::Bilinear => put_bilin_rust(dst, dst_stride, src, src_stride, w, h, mx, my, bd),
        _ => put_8tap_rust(dst, dst_stride, src, src_stride, w, h, mx, my, hv, bd),
    }
}
...
unsafe extern "C" fn put_scaled_c_erased<BD: BitDepth, const FILTER: usize>(
    ...
    match filter {
        Filter2d::Bilinear => {
            put_bilin_scaled_rust(dst, dst_stride, src, src_stride, w, h, mx, my, dx, dy, bd)
        }
        _ => put_8tap_scaled_rust(
            dst, dst_stride, src, src_stride, w, h, mx, my, dx, dy, hv, bd,
        ),
    }
}
...
unsafe extern "C" fn prep_c_erased<BD: BitDepth, const FILTER: usize>(
    ...
    let bd = BD::from_c(bitdepth_max);
    match filter {
        Filter2d::Bilinear => prep_bilin_rust(tmp, src, src_stride, w, h, mx, my, bd),
        _ => prep_8tap_rust(tmp, src, src_stride, w, h, mx, my, hv, bd),
    }
}
...
unsafe extern "C" fn prep_scaled_c_erased<BD: BitDepth, const FILTER: usize>(
    ...
    match filter {
        Filter2d::Bilinear => {
            prep_bilin_scaled_rust(tmp, src, src_stride, w, h, mx, my, dx, dy, bd)
        }
        _ => prep_8tap_scaled_rust(tmp, src, src_stride, w, h, mx, my, dx, dy, hv, bd),
    }
}

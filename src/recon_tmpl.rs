...
unsafe extern "C" fn read_golomb(msac: *mut MsacContext) -> libc::c_uint {
    ...
    loop {
        ...
        val = (val << 1 as libc::c_int)
            .wrapping_add(dav1d_msac_decode_bool_equi_sse2(msac));
    }
...
unsafe extern "C" fn decode_coefs(
    ...
    match tx2dszctx {
        ...
        4 => {
            let eob_bin_cdf_3: *mut uint16_t = ((*ts)
                ...
                .as_mut_ptr();
            eob_bin = ((*ts).msac.symbol_adapt16)
                .expect(
                    "non-null function pointer",
                )(
                &mut (*ts).msac,
    ...
    if eob_bin > 1 as libc::c_int {
        let eob_hi_bit_cdf: *mut uint16_t = ((*ts)
            ...
            .as_mut_ptr();
        let eob_hi_bit: libc::c_int = dav1d_msac_decode_bool_adapt_sse2(
            &mut (*ts).msac,
            eob_hi_bit_cdf,
        ) as libc::c_int;
        if dbg != 0 {
    ...
    if dc_tok == 0 {
        ...
        dc_sign_cdf = ((*ts).cdf.coef.dc_sign[chroma as usize][dc_sign_ctx as usize])
            .as_mut_ptr();
        dc_sign = dav1d_msac_decode_bool_adapt_sse2(&mut (*ts).msac, dc_sign_cdf)
            as libc::c_int;
        if dbg != 0 {
            printf(
    ...
    match current_block {
        17466762532672302249 => {
            let ac_dq: libc::c_uint = *dq_tbl.offset(1 as libc::c_int as isize)
                as libc::c_uint;
            loop {
                let sign: libc::c_int = dav1d_msac_decode_bool_equi_sse2(&mut (*ts).msac)
                    as libc::c_int;
                if dbg != 0 {
                    printf(
                        b"Post-sign[%d=%d]: r=%d\n\0" as *const u8
        ...
        1856052624043210190 => {
            let ac_dq_0: libc::c_uint = *dq_tbl.offset(1 as libc::c_int as isize)
                as libc::c_uint;
            loop {
                let sign_0: libc::c_int = dav1d_msac_decode_bool_equi_sse2(
                    &mut (*ts).msac,
                ) as libc::c_int;
                if dbg != 0 {
                    printf(

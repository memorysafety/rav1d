...
unsafe extern "C" fn prep_c(
    ...
    let intermediate_bits: libc::c_int = 4i32;
    loop {
        let mut x: libc::c_int = 0i32;
        while x < w {
            *tmp.offset(x as isize) =
                (((*src.offset(x as isize) as libc::c_int) << intermediate_bits) - 0i32) as int16_t;
            x += 1;
        }
...
unsafe extern "C" fn avg_c(
    ...
    let rnd: libc::c_int = ((1i32) << intermediate_bits) + 0i32 * 2i32;
    loop {
        let mut x: libc::c_int = 0i32;
        while x < w {
...
unsafe extern "C" fn w_avg_c(
    ...
    let rnd: libc::c_int = ((8i32) << intermediate_bits) + 0i32 * 16i32;
    loop {
        let mut x: libc::c_int = 0i32;
        while x < w {
            *dst.offset(x as isize) = iclip_u8(
...
unsafe extern "C" fn mask_c(
    ...
    let rnd: libc::c_int = ((32i32) << intermediate_bits) + 0i32 * 64i32;
    loop {
        let mut x: libc::c_int = 0i32;
        while x < w {
            *dst.offset(x as isize) = iclip_u8(
...
unsafe extern "C" fn w_mask_444_c(
    ...
    w_mask_c(dst, dst_stride, tmp1, tmp2, w, h, mask, sign, 0i32, 0i32);
}
unsafe extern "C" fn w_mask_422_c(
    ...
    w_mask_c(dst, dst_stride, tmp1, tmp2, w, h, mask, sign, 1i32, 0i32);
}
unsafe extern "C" fn w_mask_420_c(
    ...
    w_mask_c(dst, dst_stride, tmp1, tmp2, w, h, mask, sign, 1i32, 1i32);
}
unsafe extern "C" fn warp_affine_8x8_c(
    ...
    let mut mid_ptr: *mut int16_t = mid.as_mut_ptr();
    src = src.offset(-((3i64 * src_stride) as isize));
    let mut y: libc::c_int = 0i32;
    while y < 15i32 {
        let mut x: libc::c_int = 0i32;
...
unsafe extern "C" fn emu_edge_c(
    ...
    ref_0 = ref_0.offset(
        (iclip(y as libc::c_int, 0i32, ih as libc::c_int - 1i32) as libc::c_long * ref_stride
    ...
    let mut blk: *mut pixel = dst.offset((top_ext as libc::c_long * dst_stride) as isize);

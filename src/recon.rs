...
// This is used for lo_ctx, which is used for a lookup into lo_cdf. lo_cdf corresponds
// roughly to Coeff_Base_Cdf in the spec, rav1d/dav1d uses a  maximum value is 40 (for
// an array size of 41), whereas the spec has the array being 42 elements.
...
fn get_lo_ctx(
    ...
    // `36 + 4 == 40`, so the unwrap should be elided
    LoCdfIndex::new(lo_ctx).unwrap()
}
    ...
    fn decode_coefs_class<const TX_CLASS: usize, BD: BitDepth>(
        ...
        match tx_class {
            TxClass::TwoD => {
                ...
                //
                // Therfore, we have the maximum `end` value as
                //  `32 * (32 + 2) = 1088`
        ...
        // `.fill(0)` will become a call the the platform C library's `bzero` or
        // `memset` function. That is likely to be a version optimised for the specific
        // CPU with appropriate vectoring.
        //
        // The value of `end` is known statically to not exceed the size of `levels`, so
        // there's also no bounds check.
        levels[..end].fill(0);

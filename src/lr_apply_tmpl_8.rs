...
unsafe extern "C" fn lr_stripe(
    ...
    let mut lpf: *const pixel = ((*f).lf.lr_lpf_line[plane as usize])
        .offset(
            ((have_tt * (sby * ((4i32) << (*(*f).seq_hdr).sb128) - 4i32)) as libc::c_long * stride)
                as isize,
        )
        .offset(x as isize);
    ...
    if (*lr).type_0 as libc::c_int == DAV1D_RESTORATION_WIENER as libc::c_int {
        ...
        *fresh2 = (*lr).filter_h[2usize] as int16_t;
        ...
        (*filter.offset(0isize))[3usize] = (-((*filter.offset(0isize))[0usize] as libc::c_int
            ...
            * 2i32) as int16_t;
        let ref mut fresh3 = (*filter.offset(1isize))[6usize];
        *fresh3 = (*lr).filter_v[0usize] as int16_t;
        (*filter.offset(1isize))[0usize] = *fresh3;
        let ref mut fresh4 = (*filter.offset(1isize))[5usize];
        *fresh4 = (*lr).filter_v[1usize] as int16_t;
        (*filter.offset(1isize))[1usize] = *fresh4;
        let ref mut fresh5 = (*filter.offset(1isize))[4usize];
        *fresh5 = (*lr).filter_v[2usize] as int16_t;
        (*filter.offset(1isize))[2usize] = *fresh5;
    ...
    while y + stripe_h <= row_h {
        ...
        lpf = lpf.offset((4i64 * stride) as isize);
    }
}
unsafe extern "C" fn backup4xU(
    ...
    while u > 0i32 {
        memcpy(dst as *mut libc::c_void, src as *const libc::c_void, 4u64);
        u -= 1;
        dst = dst.offset(1);
...
pub unsafe extern "C" fn dav1d_lr_sbrow_8bpc(
    ...
    if restore_planes & LR_RESTORE_Y as libc::c_int != 0 {
        ...
        lr_sbrow(
            f,
            (*dst.offset(0isize))
                .offset(-((offset_y as libc::c_long * *dst_stride.offset(0isize)) as isize)),
            y_stripe,
            w,
            h,
            row_h,
    ...
    if restore_planes & (LR_RESTORE_U as libc::c_int | LR_RESTORE_V as libc::c_int) != 0 {
        ...
        if restore_planes & LR_RESTORE_U as libc::c_int != 0 {
            lr_sbrow(
                f,
                (*dst.offset(1isize))
                    .offset(-((offset_uv as libc::c_long * *dst_stride.offset(1isize)) as isize)),
                y_stripe_0,
                w_0,
                h_0,
                row_h_0,

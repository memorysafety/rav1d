...
unsafe extern "C" fn backup2lines(
    ...
    let y_stride: ptrdiff_t = *stride.offset(0isize);
    if y_stride < 0i64 {
        memcpy(
    ...
    if layout != DAV1D_PIXEL_LAYOUT_I400 {
        let uv_stride: ptrdiff_t = *stride.offset(1isize);
        if uv_stride < 0i64 {
            ...
            memcpy(
                ...
                (-2i64 * uv_stride) as libc::c_ulong,
            );
            memcpy(
...
unsafe extern "C" fn backup2x8(
    ...
    if flag & BACKUP_2X8_Y != 0 {
        ...
        while y < 8i32 {
            ...
            y += 1;
            y_off += *src_stride.offset(0isize);
        }
    }
    ...
    while y_0 < 8i32 >> ss_ver {
        ...
        y_0 += 1;
        y_off += *src_stride.offset(1isize);
    }
}
...
pub unsafe extern "C" fn dav1d_cdef_brow_8bpc(
    ...
    let bitdepth_min_8: libc::c_int = if 8i32 == 8i32 {
        0i32
    } else {
        (*f).cur.p.bpc - 8i32
    };
    ...
    let y_stride: ptrdiff_t = (*f).cur.stride[0usize];
    let uv_stride: ptrdiff_t = (*f).cur.stride[1usize];
    ...
    while by < by_end {
        ...
        ptrs[0usize] = (ptrs[0usize]).offset((8i64 * (*f).cur.stride[0usize]) as isize);
        ptrs[1usize] = (ptrs[1usize]).offset((8i64 * (*f).cur.stride[1usize] >> ss_ver) as isize);
        ptrs[2usize] = (ptrs[2usize]).offset((8i64 * (*f).cur.stride[1usize] >> ss_ver) as isize);

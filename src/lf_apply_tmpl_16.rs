...
unsafe extern "C" fn backup_lpf(
    ...
    src = src.offset(((stripe_h - 2i32) as libc::c_long * PXSTRIDE(src_stride)) as isize);
    if (*(*f).c).n_tc == 1u32 {
        if row != 0 {
            let top: libc::c_int = (4i32) << sb128;
            memcpy(
                &mut *dst.offset(
                    ((PXSTRIDE as unsafe extern "C" fn(ptrdiff_t) -> ptrdiff_t)(dst_stride) * 0i64)
                        as isize,
                ) as *mut pixel as *mut libc::c_void,
                &mut *dst.offset(
                    ((PXSTRIDE as unsafe extern "C" fn(ptrdiff_t) -> ptrdiff_t)(dst_stride)
                        * top as libc::c_long) as isize,
                ) as *mut pixel as *const libc::c_void,
                (dst_w << 1i32) as libc::c_ulong,
            );
            memcpy(
                &mut *dst.offset(
                    ((PXSTRIDE as unsafe extern "C" fn(ptrdiff_t) -> ptrdiff_t)(dst_stride) * 1i64)
                        as isize,
                ) as *mut pixel as *mut libc::c_void,
                &mut *dst.offset(
                    ((PXSTRIDE as unsafe extern "C" fn(ptrdiff_t) -> ptrdiff_t)(dst_stride)
                        * (top + 1i32) as libc::c_long) as isize,
                ) as *mut pixel as *const libc::c_void,
                (dst_w << 1i32) as libc::c_ulong,
            );
            memcpy(
                &mut *dst.offset(
                    ((PXSTRIDE as unsafe extern "C" fn(ptrdiff_t) -> ptrdiff_t)(dst_stride) * 2i64)
                        as isize,
                ) as *mut pixel as *mut libc::c_void,
                &mut *dst.offset(
                    ((PXSTRIDE as unsafe extern "C" fn(ptrdiff_t) -> ptrdiff_t)(dst_stride)
                        * (top + 2i32) as libc::c_long) as isize,
                ) as *mut pixel as *const libc::c_void,
                (dst_w << 1i32) as libc::c_ulong,
            );
            memcpy(
                &mut *dst.offset(
                    ((PXSTRIDE as unsafe extern "C" fn(ptrdiff_t) -> ptrdiff_t)(dst_stride) * 3i64)
                        as isize,
                ) as *mut pixel as *mut libc::c_void,
                &mut *dst.offset(
                    ((PXSTRIDE as unsafe extern "C" fn(ptrdiff_t) -> ptrdiff_t)(dst_stride)
                        * (top + 3i32) as libc::c_long) as isize,
                ) as *mut pixel as *const libc::c_void,
                (dst_w << 1i32) as libc::c_ulong,
            );
        }
        dst = dst.offset((4i64 * PXSTRIDE(dst_stride)) as isize);
    }
    if lr_backup != 0 && (*(*f).frame_hdr).width[0usize] != (*(*f).frame_hdr).width[1usize] {
        while row + stripe_h <= row_h {
            ...
            src = src.offset((stripe_h as libc::c_long * PXSTRIDE(src_stride)) as isize);
            dst = dst.offset((n_lines as libc::c_long * PXSTRIDE(dst_stride)) as isize);
            if n_lines == 3i32 {
                memcpy(
        ...
        while row + stripe_h <= row_h {
            ...
            row += stripe_h;
            stripe_h = 64i32 >> ss_ver;
            src = src.offset(((stripe_h - 4i32) as libc::c_long * PXSTRIDE(src_stride)) as isize);
        }
    };
}
#[no_mangle]
pub unsafe extern "C" fn dav1d_copy_lpf_16bpc(
    ...
    let dst: [*mut pixel; 3] = [
        ((*f).lf.lr_lpf_line[0usize])
            .offset((tt_off as libc::c_long * PXSTRIDE(*lr_stride.offset(0isize))) as isize),
        ((*f).lf.lr_lpf_line[1usize])
            .offset((tt_off as libc::c_long * PXSTRIDE(*lr_stride.offset(1isize))) as isize),
    ...
    if (*(*f).seq_hdr).cdef != 0 || restore_planes & LR_RESTORE_Y as libc::c_int != 0 {
        ...
        if restore_planes & LR_RESTORE_Y as libc::c_int != 0 || resize == 0 {
            backup_lpf(
                ...
                *lr_stride.offset(0isize),
                (*src.offset(0isize)).offset(
                    -((offset as libc::c_long * PXSTRIDE(*src_stride.offset(0isize))) as isize),
                ),
        ...
        if have_tt != 0 && resize != 0 {
            let cdef_off_y: ptrdiff_t =
                (sby * 4i32) as libc::c_long * PXSTRIDE(*src_stride.offset(0isize));
            backup_lpf(
                f,
        ...
        && (*f).cur.p.layout != DAV1D_PIXEL_LAYOUT_I400
    {
        ...
        let cdef_off_uv: ptrdiff_t =
            (sby * 4i32) as libc::c_long * PXSTRIDE(*src_stride.offset(1isize));
                ...
                backup_lpf(
                    ...
                    *lr_stride.offset(1isize),
                    (*src.offset(1isize)).offset(
                        -((offset_uv as libc::c_long * PXSTRIDE(*src_stride.offset(1isize)))
                            as isize),
                    ),
...
unsafe extern "C" fn filter_plane_rows_y(
    ...
    while y < endy4 {
        ...
        y += 1;
        dst = dst.offset((4i64 * PXSTRIDE(ls)) as isize);

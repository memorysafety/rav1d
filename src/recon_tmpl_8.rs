...
unsafe fn decode_coefs(
    ...
    let mut eob_bin = 0;
    let tx2dszctx = std::cmp::min((*t_dim).lw as libc::c_int, TX_32X32 as libc::c_int)
        + std::cmp::min((*t_dim).lh as libc::c_int, TX_32X32 as libc::c_int);
    ...
    let hi_cdf: *mut [uint16_t; 4] = ((*ts).cdf.coef.br_tok
        [std::cmp::min((*t_dim).ctx as libc::c_int, 3 as libc::c_int) as usize][chroma as usize])
        .as_mut_ptr();
    ...
    if eob != 0 {
        ...
        let sw = std::cmp::min((*t_dim).w as libc::c_int, 8 as libc::c_int);
        let sh = std::cmp::min((*t_dim).h as libc::c_int, 8 as libc::c_int);
                    ...
                    if dbg != 0 {
                        printf(
                            b"Post-hi_tok[%d][%d][%d][%d=%d=%d]: r=%d\n\0" as *const u8
                                as *const libc::c_char,
                            std::cmp::min((*t_dim).ctx as libc::c_int, 3 as libc::c_int),
                            chroma,
                            ctx,
                            eob,
                            rc,
            ...
            != 0
    {
        ...
        if (*t).frame_thread.pass != 0 {
            ...
            (*ts).frame_thread[p as usize].cf = ((*ts).frame_thread[p as usize].cf as *mut coef)
                .offset(
                    (std::cmp::min((*t_dim).w as libc::c_int, 8 as libc::c_int)
                        * std::cmp::min((*t_dim).h as libc::c_int, 8 as libc::c_int)
                        * 16) as isize,
                ) as *mut libc::c_void;
        ...
        if (*t).frame_thread.pass != 2 as libc::c_int {
            ...
            CaseSet::<16, true>::many(
                [&mut (*t).l, &mut *(*t).a],
                [
                    std::cmp::min(txh, (*f).bh - (*t).by) as usize,
                    std::cmp::min(txw, (*f).bw - (*t).bx) as usize,
                ],
...
pub unsafe extern "C" fn dav1d_read_coef_blocks_8bpc(
    ...
    let mut init_y = 0;
    while init_y < h4 {
        let sub_h4 = std::cmp::min(h4, 16 + init_y);
        let mut init_x = 0;
        while init_x < w4 {
            let sub_w4 = std::cmp::min(w4, init_x + 16);
                    ...
                    if (*b).intra == 0 {
                        ...
                        *fresh4 = decode_coefs(
                        ...
                        CaseSet::<16, true>::many(
                            [&mut (*t).l, &mut *(*t).a],
                            [
                                std::cmp::min((*t_dim).h as i32, (*f).bh - (*t).by) as usize,
                                std::cmp::min((*t_dim).w as i32, (*f).bw - (*t).bx) as usize,
                            ],
            ...
            (*t).by -= y;
            if !(has_chroma == 0) {
                let sub_ch4 = std::cmp::min(ch4, init_y + 16 >> ss_ver);
                let sub_cw4 = std::cmp::min(cw4, init_x + 16 >> ss_hor);
                let mut pl = 0;
                while pl < 2 {
                    y = init_y >> ss_ver;
                    (*t).by += init_y;
...
unsafe extern "C" fn obmc(
                ...
                >= 16)
    {
        let mut i = 0;
        let mut x = 0;
        while x < w4 && i < std::cmp::min(*b_dim.offset(2) as libc::c_int, 4 as libc::c_int) {
            ...
            if (*a_r).0.r#ref.r#ref[0] as libc::c_int > 0 {
                let ow4 = std::cmp::min(step4, *b_dim.offset(0) as libc::c_int);
                let oh4 = std::cmp::min(*b_dim.offset(1) as libc::c_int, 16 as libc::c_int) >> 1;
                res = mc(
                    t,
                    lap,
                    0 as *mut int16_t,
    ...
    if (*t).bx > (*(*t).ts).tiling.col_start {
        let mut i_0 = 0;
        let mut y = 0;
        while y < h4 && i_0 < std::cmp::min(*b_dim.offset(3) as libc::c_int, 4 as libc::c_int) {
            ...
            if (*l_r).0.r#ref.r#ref[0] as libc::c_int > 0 {
                let ow4_0 = std::cmp::min(*b_dim.offset(0) as libc::c_int, 16 as libc::c_int) >> 1;
                let oh4_0 = std::cmp::min(step4_0, *b_dim.offset(1) as libc::c_int);
                res = mc(
                    t,
                    lap,
                    0 as *mut int16_t,
...
pub unsafe extern "C" fn dav1d_recon_b_inter_8bpc(
    ...
    while init_y < bh4 {
        ...
        while init_x < bw4 {
            ...
            y = init_y;
            (*t).by += init_y;
            while y < std::cmp::min(h4, init_y + 16) {
                let mut x;
                let mut x_off = (init_x != 0) as libc::c_int;
                x = init_x;
                (*t).bx += init_x;
                while x < std::cmp::min(w4, init_x + 16) {
                    read_coef_tree(
                        t,
                        bs,
                        b,
            ...
            if has_chroma != 0 {
                ...
                while pl_8 < 2 {
                    ...
                    y = init_y >> ss_ver;
                    (*t).by += init_y;
                    while y < std::cmp::min(ch4, init_y + 16 >> ss_ver) {
                        let mut x_0;
                        x_0 = init_x >> ss_hor;
                        (*t).bx += init_x;
                        while x_0 < std::cmp::min(cw4, init_x + 16 >> ss_hor) {
                            let cf: *mut coef;
                            let eob;
...
pub unsafe extern "C" fn dav1d_filter_sbrow_cdef_8bpc(tc: *mut Dav1dTaskContext, sby: libc::c_int) {
    ...
    let end = std::cmp::min(start + n_blks, (*f).bh);
...
pub unsafe extern "C" fn dav1d_filter_sbrow_resize_8bpc(
    ...
    while pl < 1 + 2 * has_chroma {
        ...
        ((*(*f).dsp).mc.resize).expect("non-null function pointer")(
            ...
            src_stride,
            dst_w,
            std::cmp::min(img_h, h_end) + h_start,
            src_w,

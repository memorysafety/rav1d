...
unsafe fn decode_b_inner(
    ...
    if b.skip == 0 {
        ...
        let cur_idx = t.cur_sb_cdef_idx.unwrap() + idx;
        if cdef_idx[cur_idx].load(Ordering::Relaxed) == -1 {
            ...
            if debug_block_info!(f, t) {
                println!(
                    "Post-cdef_idx[{}]: r={}",
                    cdef_idx[t.cur_sb_cdef_idx.unwrap()].load(Ordering::Relaxed),
                    ts.msac.rng
                );
            }
        }
    ...
    if t.frame_thread.pass == 1 && b.intra == 0 && frame_hdr.frame_type.is_inter_or_switch() {
        ...
        let lowest_px = &mut f.lowest_pixel_mem.index_mut(ts.lowest_pixel + sby as usize);
...
unsafe fn setup_tile(
    ...
    for p in 0..3 {
        ...
        let lr = lr_ref.try_write().unwrap();
...
pub(crate) unsafe fn rav1d_decode_tile_sbrow(
    ...
    for bx in (ts.tiling.col_start..ts.tiling.col_end).step_by(sb_step as usize) {
        ...
        if root_bl == BlockLevel::Bl128x128 {
            ...
            t.cur_sb_cdef_idx = Some(0);
        } else {
            let cdef_idx = &cdef_idx[(((t.bx & 16) >> 4) + ((t.by & 16) >> 3)) as usize..];
            cdef_idx[0].store(-1, Ordering::Relaxed);
            t.cur_sb_cdef_idx = Some((((t.bx & 16) >> 4) + ((t.by & 16) >> 3)) as usize)
        }
        let frame_hdr = &***f.frame_hdr.as_ref().unwrap();
        // Restoration filter
        for p in 0..3 {
            ...
            if frame_hdr.size.width[0] != frame_hdr.size.width[1] {
                ...
                read_restoration_info(&mut *t.ts, &mut lr, p, frame_type, debug_block_info!(f, t));
            }
        }
        decode_sb(c, t, f, root_bl, EdgeIndex::root())?;
        if t.bx & 16 != 0 || f.seq_hdr().sb128 != 0 {
    ...
    let start_y = (align_h * tile_col + t.by) as usize;
    f.lf.tx_lpf_right_edge.copy_from_slice_y(
        start_y..start_y + sb_step as usize,
        &t.l.tx_lpf_y.0[(t.by & 16) as usize..][..sb_step as usize],
    );
    let ss_ver = (f.cur.p.layout == Rav1dPixelLayout::I420) as c_int;
    align_h >>= ss_ver;
    let start_uv = (align_h * tile_col + (t.by >> ss_ver)) as usize;

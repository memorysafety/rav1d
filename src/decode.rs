...
pub unsafe extern "C" fn dav1d_decode_frame_init(f: *mut Dav1dFrameContext) -> libc::c_int {
        ...
        || (*f).sbh != (*f).lf.cdef_buf_sbh
    {
        ...
        (*f).lf.cdef_buf_plane_sz[0] = y_stride as libc::c_int * (*f).sbh * 4 as libc::c_int;
        (*f).lf.cdef_buf_plane_sz[1] = uv_stride as libc::c_int * (*f).sbh * 8 as libc::c_int;
        ...
        || uv_stride * num_lines as isize * 2 != (*f).lf.lr_buf_plane_sz[1] as isize
    {
        ...
        (*f).lf.lr_buf_plane_sz[1] = uv_stride as libc::c_int * num_lines * 2 as libc::c_int;
    }
    ...
    if re_sz != (*f).lf.re_sz {
        ...
        (*f).lf.tx_lpf_right_edge[1] =
            ((*f).lf.tx_lpf_right_edge[0]).offset((re_sz * 32 as libc::c_int) as isize);
        (*f).lf.re_sz = re_sz;
    }
...
pub unsafe extern "C" fn dav1d_submit_frame(c: *mut Dav1dContext) -> libc::c_int {
    ...
    if (*(*f).frame_hdr).frame_type as libc::c_uint & 1 as libc::c_int as libc::c_uint != 0 {
        ...
        while i < 7 as libc::c_int {
            ...
            if ((*c).refs[refidx as usize].p.p.data[0]).is_null()
                || ((*(*f).frame_hdr).width[0] * 2 as libc::c_int)
            ...
            ref_coded_width[i as usize] = (*(*c).refs[refidx as usize].p.p.frame_hdr).width[0];
            if (*(*f).frame_hdr).width[0] != (*c).refs[refidx as usize].p.p.p.w
                || (*(*f).frame_hdr).height != (*c).refs[refidx as usize].p.p.p.h
            {
                (*f).svc[i as usize][0].scale = (((*c).refs[refidx as usize].p.p.p.w
                    << 14 as libc::c_int)
                    + ((*(*f).frame_hdr).width[0] >> 1 as libc::c_int))
                    / (*(*f).frame_hdr).width[0];
                (*f).svc[i as usize][1].scale = (((*c).refs[refidx as usize].p.p.p.h
                    << 14 as libc::c_int)
                    + ((*(*f).frame_hdr).height >> 1 as libc::c_int))
                    / (*(*f).frame_hdr).height;
                (*f).svc[i as usize][0].step =
                    (*f).svc[i as usize][0].scale + 8 as libc::c_int >> 4 as libc::c_int;
                (*f).svc[i as usize][1].step =
                    (*f).svc[i as usize][1].scale + 8 as libc::c_int >> 4 as libc::c_int;
            } else {
    ...
    if (*(*f).frame_hdr).width[0] != (*(*f).frame_hdr).width[1] {
        (*f).resize_step[0] = (((*f).cur.p.w << 14 as libc::c_int)
    ...
    (*f).w4 = (*(*f).frame_hdr).width[0] + 3 as libc::c_int >> 2 as libc::c_int;
    (*f).h4 = (*(*f).frame_hdr).height + 3 as libc::c_int >> 2 as libc::c_int;
    (*f).bw =
        ((*(*f).frame_hdr).width[0] + 7 as libc::c_int >> 3 as libc::c_int) << 1 as libc::c_int;

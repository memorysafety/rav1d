...
unsafe extern "C" fn find_matching_ref(
                ...
                == -(1 as libc::c_int))
    {
        ...
        *fresh6 = (*fresh6 as libc::c_ulonglong | (1 as libc::c_ulonglong) << 32 as libc::c_int)
            as uint64_t;
        count += 1;
        if count >= 8 as libc::c_int {
            return;
                ...
                == -(1 as libc::c_int))
    {
        ...
        *fresh7 = (*fresh7 as libc::c_ulonglong | (1 as libc::c_ulonglong) << 32 as libc::c_int)
            as uint64_t;
    }
}
        ...
        while j <= (*((*f).tile).offset(i as isize)).end {
            ...
            if j == (*((*f).tile).offset(i as isize)).end {
                ...
                while k < (*(*f).frame_hdr).tiling.n_bytes {
                    let fresh37 = data;
                    data = data.offset(1);
                    tile_sz |= ((*fresh37 as libc::c_uint)
    ...
    if (*c).n_fc > 1 as libc::c_int as libc::c_uint {
            ...
            || ::core::intrinsics::atomic_load(&mut (*f).task_thread.error as *mut atomic_int) != 0
        {
            ...
            if first.wrapping_add(1 as libc::c_uint) < (*c).n_fc {
                ::core::intrinsics::atomic_xadd(&mut (*c).task_thread.first, 1 as libc::c_uint);
            } else {
                ::core::intrinsics::atomic_store(

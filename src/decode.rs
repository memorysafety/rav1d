...
unsafe fn decode_b(
    ...
    if b.intra != 0 {
        ...
        if b.skip_mode != 0 {
            ...
            *b.comp_type_mut() = COMP_INTER_NONE as u8;
            if let Some(seg) = seg.filter(|seg| seg.r#ref > 0) {
                b.r#ref_mut()[0] = seg.r#ref as i8 - 1;
                ...
                if dav1d_msac_decode_bool_adapt(&mut ts.msac, &mut ts.cdf.m.r#ref[0][ctx1 as usize])
                {
                    ...
                    if dav1d_msac_decode_bool_adapt(
                        ...
                        b.r#ref_mut()[0] = (4 as libc::c_uint).wrapping_add(
                            dav1d_msac_decode_bool_adapt(
                                &mut ts.msac,
                    ...
                    if dav1d_msac_decode_bool_adapt(
                        ...
                        let ctx3 =
                            av1_get_fwd_ref_2_ctx(&*t.a, &t.l, by4, bx4, have_top, have_left);
                        b.r#ref_mut()[0] = (2 as libc::c_uint).wrapping_add(
                            dav1d_msac_decode_bool_adapt(
                                &mut ts.msac,
                        ...
                        b.r#ref_mut()[0] = dav1d_msac_decode_bool_adapt(
                            &mut ts.msac,
        ...
        *b.filter2d_mut() = dav1d_filter_2d[filter[1] as usize][filter[0] as usize];
        ...
        if frame_hdr.loopfilter.level_y[0] != 0 || frame_hdr.loopfilter.level_y[1] != 0 {
            ...
            let lf_lvls = &*(*(*(*(ts.lflvl).offset(b.seg_id as isize)).as_ptr().offset(0))
                .as_ptr()
                .offset(*b.r#ref_mut().as_mut_ptr().offset(0) as isize + 1))
            .as_ptr()
            .offset((is_globalmv == 0) as isize) as *const u8
                as *const [[u8; 2]; 8];
    ...
    if b.skip == 0 {
        ...
        let mask = !(0 as libc::c_uint) >> 32 - bw4 << (bx4 & 15);
        let bx_idx = (bx4 & 16) >> 4;
        let mut y = 0;
        while y < bh4 {

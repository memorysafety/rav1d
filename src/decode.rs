...
use crate::src::ctx::case_set_many;
extern "C" {
    fn memcpy(
        _: *mut libc::c_void,
        _: *const libc::c_void,
...
unsafe fn decode_b(
    ...
    if t.frame_thread.pass == 2 as libc::c_int {
        if (*b).intra != 0 {
            ...
            case_set_many([bh4, bw4], [by4, bx4], [&mut t.l, &mut *t.a], |case, dir| {
                case.set(&mut dir.mode, y_mode_nofilt);
                case.set(&mut dir.intra, 1);
            });
            ...
            if has_chroma != 0 {
                case_set_many([cbh4, cbw4], [cby4, cbx4], [&mut t.l, &mut *t.a], |case, dir| {
                    case.set(&mut dir.uvmode, (*b).c2rust_unnamed.c2rust_unnamed.uv_mode);
                });
            }
        } else {
            if (*(*f).frame_hdr).frame_type as libc::c_uint
            ...
            let filter: *const uint8_t = (dav1d_filter_dir[(*b)
                ...
                .as_ptr();
            case_set_many([bh4, bw4], [by4, bx4], [&mut t.l, &mut *t.a], |case, dir| {
                case.set(&mut dir.filter[0], *filter.offset(0));
                case.set(&mut dir.filter[1], *filter.offset(1));
                case.set(&mut dir.intra, 0);
            });
            if (*(*f).frame_hdr).frame_type as libc::c_uint
                & 1 as libc::c_int as libc::c_uint != 0
            {
            ...
            if has_chroma != 0 {
                case_set_many([cbh4, cbw4], [cby4, cbx4], [&mut t.l, &mut *t.a], |case, dir| {
                    case.set(&mut dir.uvmode, DC_PRED);
                });
            }
        }
        return 0 as libc::c_int;

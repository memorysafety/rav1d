            ...
            if wd >= 16i32 && flat8out & flat8in != 0 {
                ...
                *dst.offset((strideb * 1i64) as isize) =
                ...
                if hev != 0 {
                    ...
                    *dst.offset((strideb * -1i64) as isize) =
                        iclip(p0 + f2, 0i32, bitdepth_max) as pixel;
                    *dst.offset((strideb * 0i64) as isize) =
                        iclip(q0 - f1, 0i32, bitdepth_max) as pixel;
                } else {
                    ...
                    *dst.offset((strideb * -1i64) as isize) =
                        iclip(p0 + f2_0, 0i32, bitdepth_max) as pixel;
                    *dst.offset((strideb * 0i64) as isize) =
                        iclip(q0 - f1_0, 0i32, bitdepth_max) as pixel;
                    f_0 = f1_0 + 1i32 >> 1i32;
                    *dst.offset((strideb * -2i64) as isize) =
                        iclip(p1 + f_0, 0i32, bitdepth_max) as pixel;
                    *dst.offset((strideb * 1i64) as isize) =
                        iclip(q1 - f_0, 0i32, bitdepth_max) as pixel;
                }
            }
        }
        i += 1;
...
unsafe extern "C" fn loop_filter_h_sb128y_c(
    ...
    while vm & !y.wrapping_sub(1u32) != 0 {
        ...
        y <<= 1i32;
        dst = dst.offset((4i64 * PXSTRIDE(stride)) as isize);

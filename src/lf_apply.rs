...
pub(crate) unsafe fn rav1d_copy_lpf<BD: BitDepth>(
    ...
    if seq_hdr.cdef != 0 || restore_planes & LR_RESTORE_Y as c_int != 0 {
        ...
        if restore_planes & LR_RESTORE_Y as c_int != 0 || resize == 0 {
            backup_lpf::<BD>(
                ...
                lr_stride[0],
                src[0],
                (src_offset[0] as isize - offset as isize * BD::pxstride(src_stride[0])) as usize,
                src_stride[0],
                0,
                seq_hdr.sb128,
                y_stripe,
        ...
        && f.cur.p.layout != Rav1dPixelLayout::I400
    {
        ...
        let cdef_off_uv: ptrdiff_t = sby as isize * 4 * BD::pxstride(src_stride[1]);
...
pub(crate) unsafe fn rav1d_loopfilter_sbrow_cols<BD: BitDepth>(
    ...
    loop {
        ...
        if f.cur.p.layout != Rav1dPixelLayout::I400 {
            let uv_hmask: &mut [[u16; 2]; 2] =
                &mut lflvl[x as usize].filter_uv.nd_index_mut([0, cbx4 as usize]);
    ...
    if start_of_tile_row != 0 {
        ...
        for x in 0..f.sb128w {
            ...
            if f.cur.p.layout != Rav1dPixelLayout::I400 {
                ...
                let uv_vmask: &mut [[u16; 2]; 2] = &mut lflvl[x as usize]
                    .filter_uv
                    .nd_index_mut([1, (starty4 >> ss_ver) as usize]);
                for i in 0..cw {
                    let uv_mask: u32 = 1 << i;
    ...
    for x in 0..f.sb128w {
        filter_plane_cols_uv::<BD>(
            ...
            level_ptr,
            f.b4_stride,
            &lflvl[x as usize].filter_uv.index(0),
            pu,
            pv,
            uv_off,
            f.cur.stride[1],
...
pub(crate) unsafe fn rav1d_loopfilter_sbrow_rows<BD: BitDepth>(
    ...
    for x in 0..f.sb128w {
        filter_plane_rows_uv::<BD>(
            ...
            level_ptr,
            f.b4_stride,
            &lflvl[x as usize].filter_uv.index(1),
            pu,
            pv,
            p_offset[1] + uv_off,
            f.cur.stride[1],

...
unsafe fn create_filter_sbrow(
    ...
    if pass & 1 != 0 {
        ...
        let mut frame_thread_progress = fc.frame_thread_progress.try_write().unwrap();
        let prog_sz = ((f.sbh + 31 & !(31 as c_int)) >> 5) as usize;
        frame_thread_progress.frame.clear();
        frame_thread_progress
            .frame
            .resize_with(prog_sz, || AtomicU32::new(0));
        frame_thread_progress.copy_lpf.clear();
        frame_thread_progress
            .copy_lpf
            .resize_with(prog_sz, || AtomicU32::new(0));
        frame_thread_progress.deblock.store(0, Ordering::SeqCst);
    }
...
pub(crate) unsafe fn rav1d_task_frame_init(c: &Rav1dContext, fc: &Rav1dFrameContext, index: usize) {
...
unsafe fn get_frame_progress(fc: &Rav1dFrameContext, f: &Rav1dFrameData) -> c_int {
    ...
    let mut prog;
    let frame_thread_progress = fc.frame_thread_progress.try_read().unwrap();
    loop {
        let val: c_uint = !(frame_thread_progress.frame)[idx as usize].load(Ordering::SeqCst);
        ...
        prog = 0 as c_int;
        idx += 1;
        if !((idx as usize) < frame_thread_progress.frame.len()) {
            break;
        }
    }
    return (idx << 5 | prog) - 1;
    ...
    'outer: while !tc.task_thread.die.load(Ordering::Relaxed) {
        ...
        let (fc, t_idx, prev_t) = 'found: {
            if c.n_fc > 1 as c_uint {
                ...
                'init_tasks: for i in 0..c.n_fc {
                    let first = ttd.first.load(Ordering::SeqCst);
                    let fc = &*(c.fc).offset(first.wrapping_add(i).wrapping_rem(c.n_fc) as isize);
                    let tasks = &*fc.task_thread.tasks();
                ...
                while let Some(t_idx) = next_t {
                    ...
                    'next: {
                        ...
                        if matches!(
                            ...
                            let p1_1 = fc.frame_thread_progress.try_read().unwrap().copy_lpf
                                [(t.sby - 1 >> 5) as usize]
                                .load(Ordering::SeqCst);
        ...
        'found_unlocked: loop {
                ...
                match task_type {
                    ...
                    TaskType::DeblockCols => {
                        ...
                        if ensure_progress(
                            ...
                            t_idx,
                            TaskType::DeblockRows,
                            &fc.frame_thread_progress.try_read().unwrap().deblock,
                    ...
                    TaskType::DeblockRows => {
                        ...
                        {
                            error_0 = fc.task_thread.error.load(Ordering::SeqCst);
                            fc.frame_thread_progress.try_read().unwrap().deblock.store(
                            ...
                            fc.frame_thread_progress.try_read().unwrap().copy_lpf
                                [(sby >> 5) as usize]
                                .fetch_or((1 as c_uint) << (sby & 31), Ordering::SeqCst);
                            // CDEF needs the top buffer to be saved by lr_copy_lpf of the
                            // previous sbrow
                            if sby != 0 {
                                let prog_1 = fc.frame_thread_progress.try_read().unwrap().copy_lpf
                                    [(sby - 1 >> 5) as usize]
                                    .load(Ordering::SeqCst);
            ...
            if t.type_0 == TaskType::EntropyProgress {
                ...
                fc.frame_thread_progress.try_read().unwrap().entropy.store(
                    if error_0 != 0 { TILE_ERROR } else { sby + 1 },
                    Ordering::SeqCst,
                );
                if sby + 1 == sbh {
            ...
            fc.frame_thread_progress.try_read().unwrap().frame[(sby >> 5) as usize]
                .fetch_or((1 as c_uint) << (sby & 31), Ordering::SeqCst);
            {
